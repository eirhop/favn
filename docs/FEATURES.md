...
- Private local consumer projects can combine `favn` with optional plugin app path dependencies from one local checkout, for example `favn_duckdb`, without manually listing internal runtime apps. State: `solid but still private-dev`. Refs: `apps/favn/test/git_dependency_install_test.exs`, `apps/*/mix.exs`.
