...
- Pipelines are supported with direct asset targets, multi-target shorthands, additive selectors by asset, module, tag, and category, plus config, metadata, schedules, named window/source, and outputs. Cron schedules support standard 5-field expressions and 6-field expressions with a leading seconds field, and `missed: :all` catch-up is capped per schedule entry per tick to avoid unbounded high-frequency backlog submission. State: `solid but still private-dev`. Refs: `Favn.Pipeline`, `apps/favn_authoring/lib/favn/pipeline.ex`, `apps/favn/test/public_pipeline_parity_test.exs`.
...
- Public Mix tasks exist for `init`, `doctor`, `install`, `dev`, `run`, `status`, `logs`, `reload`, `stop`, `reset`, `build.runner`, `build.web`, `build.orchestrator`, and `build.single`. State: `solid but still private-dev`. Refs: `apps/favn/lib/mix/tasks/*.ex`, `apps/favn/test/mix_tasks/public_tasks_test.exs`.
...
- AI-oriented doc breadcrumbs now exist in compiled docs through `Favn.AI`, richer `Favn` function docs, and cross-links on the main authoring/local-tooling entry modules. State: `solid but still private-dev`. Refs: `apps/favn/lib/favn/ai.ex`, `apps/favn/lib/favn.ex`, `apps/favn_authoring/lib/favn/*.ex`, `apps/favn_local/lib/favn/dev.ex`, `apps/favn_core/lib/favn/manifest/generator.ex`.
...
- `mix favn.dev` starts runner, orchestrator, and web as separate local processes, stores runtime state under `.favn/runtime.json`, checks HTTP-level readiness through a dependency-free loopback-only local HTTP client, disables the local scheduler by default unless `--scheduler` or local config opts in, forwards only the supported `FAVN_ORCHESTRATOR_BOOTSTRAP_*` keys from the consumer `.env` or shell environment to the orchestrator process, passes only explicit local-dev consumer Favn config keys into the runner with consumer-root-relative database paths resolved and redacted diagnostics, and bootstraps one pinned manifest version into both runner and orchestrator with structured local API failure diagnostics. State: `solid but still private-dev`. Refs: `Favn.Dev.Stack`, `Favn.Dev.LocalHttpClient`, `Favn.Dev.RuntimeLaunch`, `Favn.Dev.ConsumerConfigTransport`, `Favn.Manifest.Version`, `apps/favn_local/lib/favn/dev/stack.ex`, `apps/favn_local/lib/favn/dev/local_http_client.ex`, `apps/favn_local/lib/favn/dev/runtime_launch.ex`, `apps/favn_local/lib/favn/dev/consumer_config_transport.ex`, `apps/favn_core/lib/favn/manifest/version.ex`.
- `mix favn.run PipelineModule` submits a pipeline run to the running local stack through the private orchestrator HTTP boundary, using generated local operator credentials and active-manifest target resolution. State: `solid but still private-dev`. Refs: `Mix.Tasks.Favn.Run`, `Favn.Dev.Run`, `apps/favn/lib/mix/tasks/favn.run.ex`, `apps/favn_local/lib/favn/dev/run.ex`.
...
- The local web UI includes an asset catalog at `/assets` and asset details at `/assets/[asset_ref]`, backed by active-manifest target data and recent runs. It supports client-side search/filtering, summary counts, asset-scoped run history, honest missing-metadata empty states for lineage/schema data not exposed by the backend yet, and a run-with-upstream action using the current orchestrator asset submission path. State: `prototype`. Refs: `web/favn_web/src/routes/assets/**`, `web/favn_web/src/lib/components/favn/AssetCatalogPage.svelte`, `web/favn_web/src/lib/components/favn/AssetDetailPage.svelte`, `web/favn_web/src/lib/server/asset_catalog_views.ts`.
