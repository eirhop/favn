...
- Pipelines are supported with direct asset targets, multi-target shorthands, additive selectors by asset, module, tag, and category, plus config, metadata, schedules, named window/source, and outputs. Cron schedules support standard 5-field expressions and 6-field expressions with a leading seconds field, and `missed: :all` catch-up is capped per scheduler tick to avoid unbounded high-frequency backlog submission. State: `solid but still private-dev`. Refs: `Favn.Pipeline`, `apps/favn_authoring/lib/favn/pipeline.ex`, `apps/favn/test/public_pipeline_parity_test.exs`.
