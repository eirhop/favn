...
- `mix favn.dev` starts runner, orchestrator, and web as separate local processes, stores runtime state under `.favn/runtime.json`, checks HTTP-level readiness through a dependency-free loopback-only local HTTP client, passes consumer Favn connection/plugin config into the runner with consumer-root-relative database paths resolved, and bootstraps one pinned manifest version into both runner and orchestrator with structured local API failure diagnostics. State: `solid but still private-dev`. Refs: `Favn.Dev.Stack`, `Favn.Dev.LocalHttpClient`, `Favn.Dev.RuntimeLaunch`, `Favn.Manifest.Version`, `apps/favn_local/lib/favn/dev/stack.ex`, `apps/favn_local/lib/favn/dev/local_http_client.ex`, `apps/favn_local/lib/favn/dev/runtime_launch.ex`, `apps/favn_core/lib/favn/manifest/version.ex`.
