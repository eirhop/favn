...
- Public SQL runtime access is supported through `Favn.SQLClient` for connect/query/execute/transaction/relation/columns usage from plain Elixir code against named `Favn.Connection` entries. The shared SQL runtime now includes connection-scoped admission control so adapters can serialize unsafe SQL operations while capable backends opt into higher or unlimited write concurrency. State: `solid but still private-dev`. Refs: `Favn.SQLClient`, `Favn.SQL.Client`, `Favn.SQL.Admission`, `apps/favn/lib/favn/sql_client.ex`, `apps/favn_sql_runtime/lib/favn/sql/client.ex`.
...
- `mix favn.dev` starts runner, orchestrator, and web as separate local processes, stores runtime state under `.favn/runtime.json`, checks HTTP-level readiness through a dependency-free loopback-only local HTTP client, and bootstraps one pinned manifest version into both runner and orchestrator with structured local API failure diagnostics. State: `solid but still private-dev`. Refs: `Favn.Dev.Stack`, `Favn.Dev.LocalHttpClient`, `Favn.Manifest.Version`, `apps/favn_local/lib/favn/dev/stack.ex`, `apps/favn_local/lib/favn/dev/local_http_client.ex`, `apps/favn_core/lib/favn/manifest/version.ex`.
