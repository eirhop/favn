...
- Public Mix tasks exist for `install`, `dev`, `run`, `status`, `logs`, `reload`, `stop`, `reset`, `build.runner`, `build.web`, `build.orchestrator`, and `build.single`. State: `solid but still private-dev`. Refs: `apps/favn/lib/mix/tasks/*.ex`, `apps/favn/test/mix_tasks/public_tasks_test.exs`.
