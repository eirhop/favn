...
- `mix favn.dev` starts runner, orchestrator, and web as separate local processes, stores runtime state under `.favn/runtime.json`, checks HTTP-level readiness, and bootstraps a manifest into runner and orchestrator with structured local API failure diagnostics. State: `solid but still private-dev`. Refs: `Favn.Dev.Stack`, `Favn.Dev.LocalHttpClient`, `apps/favn_local/lib/favn/dev/stack.ex`, `apps/favn_local/lib/favn/dev/local_http_client.ex`.
