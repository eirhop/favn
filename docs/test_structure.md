...
- Runtime config contract coverage now includes `apps/favn/test/dsl_compiler_test.exs` for asset DSL declarations, `apps/favn_core/test/manifest/serializer_test.exs` for manifest-safe serialization, `apps/favn_runner/test/worker_test.exs` for runner `ctx.config` resolution and missing-env failures, and `apps/favn_runner/test/connection/loader_test.exs` for connection env ref resolution.
