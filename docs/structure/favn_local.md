...
Breadcrumbs:
- `Mix.Tasks.Favn.Query` owns CLI startup for direct ad hoc SQL. It must start
  the Mix app and `:favn_sql_runtime` before delegating to `Favn.Dev.query/2` so
...
- `Favn.Dev.DataInspection` owns relation parsing, connection resolution, and the
  read-only SQL guardrail used by `mix favn.inspect` and `mix favn.query`.
