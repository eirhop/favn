...
`pg_pool_max_connections`, `pg_pool_enable_thread_local_cache`, timeout settings,
and the reaper-thread flag. For the current pinned DuckDB 1.5.2/Postgres
extension build, prefer a finite per-attached-database
`pg_pool_max_connections`, `pg_pool_enable_thread_local_cache: false`, and
bounded `threads`. `pg_pool_acquire_mode` is not exposed by that build, and the
deprecated `pg_connection_limit` setting is not supported. Capacity planning must
account for Favn execution concurrency, DuckLake catalog `write_concurrency`,
DuckDB `threads`, number of Postgres-backed attaches, per-catalog DuckDB Postgres
pool limits, possible force-style acquisition above those limits, and the
