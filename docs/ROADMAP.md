...
<<<<<<< feature/issue-302-scheduler-state-dto
- Audit remaining generic `PayloadCodec` persistence for idempotency and operational read-model payloads, and replace any durable database boundary that still stores reconstructed BEAM terms with explicit JSON-safe DTO records.
=======
- Audit remaining generic `PayloadCodec` persistence outside run snapshots/events and SQLite auth state, then replace each remaining durable storage area with explicit JSON-safe DTO records where the database boundary still stores reconstructed BEAM terms.
>>>>>>> main
