...
- Audit remaining generic `PayloadCodec` persistence outside run snapshots/events/scheduler-state, SQLite auth state, idempotency replay responses, and backfill read models, then replace each remaining durable storage area with explicit JSON-safe DTO records where the database boundary still stores reconstructed BEAM terms.
