...
| Workers stopped from the pre-dispatch state only | `continue_pipeline_settlement/1` also stops workers from the post-settlement state when the same dispatch ends terminal | Second reviewer finding: a node going pending and the stage then timing out its admission wait during refill in the same dispatch would have leaked the new worker | Closes the one same-dispatch leak; no behaviour change otherwise. Not covered by a dedicated test because the path needs an expired admission deadline during a batch-budget refill; the wrapper is exercised by the existing terminal-transition tests | Pending |
| "Every persist path" stops on a fenced write | Run-start, step, and terminal paths stop. Stage admission's `step_started` and `step_queued` writes still treat a fenced error as an ordinary admission failure, run their cleanup (lease release, claim failure, sibling cancellation), and the following `step_failed` write then stops the process | Second reviewer finding, pre-existing behaviour outside the three planned paths; admission failure handling is the subject of #693 and was not changed here | A stale owner can still cancel a newer owner's sibling tasks between two fenced writes; module docs and this record now say so, and the gap is noted for #693 | Pending |
| `JsonSafe` typed-map clause omits `message` and `reason` when absent | The clause always emits all four keys, defaulting message and reason to the type name | Second reviewer finding: a stored error without those keys re-encoded through the catch-all on the next snapshot write, coming back as type `map` with an inspected message. Round-trip stability is now asserted by test | None on the page; stable persisted shape | Pending |
| Dropped continuations only kill the worker | `stop_post_step_workers/1` also releases the pending node's resource-circuit permits and terminates the worker through `Task.Supervisor.terminate_child/2`, falling back to a kill only if the supervisor is gone | Second reviewer findings: permits settled at finish would otherwise wait for the probe lease after a cancel, and a raw kill produced a supervisor error report per stop | Cleaner cancellation; worker exit reason is `:shutdown` | Pending |
...
| Whole-umbrella Dialyzer (`scripts/check_elixir_static_security.sh dialyzer`) | Clean. The first push failed on two `can never succeed` guard warnings from nil-guarded telemetry fields in the orphaned-continuation fallback; the guards were removed and the callback-level routing tests now build their state from the run fixture | Static |
...
- The same-dispatch leak fix in `continue_pipeline_settlement/1` has no
  dedicated test (see deviations).
- Pre-existing: a node whose post-step settlement fails keeps node status `ok`
  in the stage's node statuses because its node result was already appended as
  successful; freshness bookkeeping then treats it as succeeded. Unchanged from
  the synchronous path and noted for follow-up.
...
| Second independent reviewer | Adversarial correctness review by a second agent (Fable 5.1, high reasoning), 2026-09-03, over the committed diff b6b2f0a3 and all surrounding code paths. Findings: same-dispatch worker leak on an expired admission deadline during refill (S1, fixed); stage-admission fenced writes run cleanup before stopping (S2, pre-existing, documented and noted for #693); typed-map error shape not round-trip stable (S3, fixed); pending permits not released on drop (M1, fixed); raw kill logs a supervisor report (M2, fixed); post-step failure keeps node status ok (M3, pre-existing, listed under not verified). Everything else was verified correct: routing and clause order, exactly-once settlement, stage bookkeeping, run snapshot consistency, persist-retry and resume, worker lifecycle, recovery, SQL, View, and tests. Verdict: not blocking; fix S1 and S3 before merge, which was done. |
