...
| Approved plan commit | Original baseline `c2d97ad1c5af3179ba72c31e6923dee80a12885e`; independently reviewed Python-removal amendment `5711c6bbf29cdf1acea755613c652a031cfe8741` (both content-identical to their pre-rebase reviewed commits) |
...
| [Worker supervisor at baseline](https://github.com/eirhop/favn/blob/c2d97ad1c5af3179ba72c31e6923dee80a12885e/apps/favn_duckdb_adbc/lib/favn_duckdb_adbc/semantic_compiler/worker.py) | Linux parent-death mechanism and the common cleanup defects above | Requires fault-injection regressions |
...
| [Existing lifecycle tests at baseline](https://github.com/eirhop/favn/blob/c2d97ad1c5af3179ba72c31e6923dee80a12885e/apps/favn_duckdb_adbc/test/semantic_compiler_lifecycle.py) | Linux success, timeout, caller loss, and supervisor death coverage | Linux `prctl` and `/proc` assertions cannot prove Darwin behavior |
...
Implementation began against the originally reviewed plan, now rebased
content-identically to `c2d97ad1`: the working tree contains the loopback
...
numbers. Counts are the eventual diff against `c2d97ad1`, excluding this record,
...
| 2026-09-23 | Rebase PR #755 again onto `origin/main` at `82446046` | Main merged PR #754's run-ownership work. `git range-diff` matches all seven PR patches exactly; the reviewed plan/amendment blobs and historical worker checksum are unchanged. The new main modifies the separate-node lifecycle fixture, so hosted qualification must rerun on the new base. |
...
| Cross-host artifact parity before acceptance | Fixed canonical fixture and byte-compare CI job are implemented; a one-time historical validator probe below establishes the old DuckDB 1.5.5 artifact digest. Hosted Linux/Mac exact-byte comparison passed on both the pre-second-rebase and rebased code heads. | The owner explicitly requested review before PR creation, so hosted CI could not provide final evidence at that review | Astra accepted the baseline provenance and CI gate; the final-head parity gate has now passed |
...
| Native worker build | `mix compile --warnings-as-errors` built C with `-Wall -Wextra -Werror` on macOS 26.5.1 arm64; target Elixir 1.20.4 and OTP 29.1. Hosted Linux and macOS production and fault-enabled worker builds passed on the rebased code head. | Both qualified hosts; not every consumer packaging environment |
| Native semantic, lifecycle and artifact tests | 31 passed together on macOS with pinned DuckDB 1.5.5 after the protocol/fault and malformed-option additions; the corrected hosted Linux native integration step and native macOS lane passed on the rebased code head. | Qualified CI slices; not exhaustive scheduling interleavings |
...
| Historical semantic parity | A one-time temporary probe imported the original worker from the content-identical baseline now at commit `c2d97ad1` (worker SHA-256 `8d33b6d4a0a327e0b4e57a98e005d8b3b35107322285d77f8760cd892fafd283`) and called its original `native` validator directly on macOS 26.5.1 with DuckDB 1.5.5 from universal ZIP SHA-256 `7b5b8915cc382d0708636fe6385c0cdad5a61c9ff8ba2638b3e2141640783155`; the Python supervisor's Linux-only ownership was not run. Unchanged `FavnAuthoring.Semantic.Builder` and `Favn.Semantic.Artifact` produced `sm_fd4123e936c378c85114d0899074b9e8340c0ae3d8397ea930a1418957ccd7d7`, exact JSON SHA-256 `78100cda23beec7137157e95c6cefe57afd7399e080fb76fdb60badce237caea`. The replacement produced the same digest on the same fixture and now asserts both golden values in CI. No historical Python is retained in the repository. | Probe validates semantic output, not historical supervisor lifetime; hosted parity passed on the rebased code head |
...
| Second hosted CI run | [35835190050](https://github.com/eirhop/favn/actions/runs/35835190050) passed quick checks, macOS native semantic/DuckLake and separate-node qualification, acceptance, slow tests, and Dialyzer. The Linux umbrella fast-test step passed, so the earlier storage timeout did not recur. The later Linux native integration step failed five lifecycle cases because GCC treated an unchecked `write` in test-only signal logging as an error when compiling the fault-enabled worker with `-Werror`; parity was skipped. The signal logger now checks the one-byte write result. Local warning-free build and all 17 lifecycle tests passed after the correction. | At that point the fault-enabled build and parity needed another run; no change to production worker behavior |
| Third hosted CI run before second rebase | [35841902266](https://github.com/eirhop/favn/actions/runs/35841902266) passed every job, including Linux fast/native integration, native macOS, separate-node acceptance, quick, slow, Dialyzer, and semantic artifact parity. The parity job's `cmp` passed and both artifact SHA-256 values were `78100cda23beec7137157e95c6cefe57afd7399e080fb76fdb60badce237caea`. | Qualifies commit `71abcb5a` against old main `c1b4d7f2`; it does not qualify the newly rebased head |
| Second rebase integration checks on macOS 26 arm64 | `git range-diff` matched all seven PR patches exactly against new main `82446046`; both reviewed plan blobs and the historical worker SHA-256 are unchanged. Warning-free compile passed, `favn_local` fast 63 passed, native semantic/DuckLake CI slice 102 passed, changed separate-node source lifecycle acceptance 1 passed with restricted PostgreSQL 18.6 role, and canonical artifact ID and JSON SHA-256 remained unchanged. | Local new-base integration checks, not hosted Linux/macOS qualification on the rebased head |
| Fourth hosted CI run on rebased code head | [35844118461](https://github.com/eirhop/favn/actions/runs/35844118461) passed every CI job at commit `e99f3f8b` against main `82446046`, including Linux fast/native integration, native macOS, separate-node acceptance, quick, slow, Dialyzer, and exact semantic parity. The parity job's `cmp` passed; both artifact SHA-256 values were `78100cda23beec7137157e95c6cefe57afd7399e080fb76fdb60badce237caea`. The control-plane image workflow [35844118520](https://github.com/eirhop/favn/actions/runs/35844118520) also passed its PR qualification jobs. | Hosted qualification of the rebased code head; only this documentation outcome update follows it |
| First rebase integration checks on macOS 26 arm64 | `mix compile --warnings-as-errors`; `favn_sql_runtime` fast 139 passed; `favn_runner` fast 287 passed; `favn_duckdb_adbc` fast 47 passed; native semantic/lifecycle/artifact 31 passed with checksum-pinned DuckDB 1.5.5; canonical artifact ID and JSON SHA-256 unchanged; formatter, Python guard, tag guard, Ruby YAML parse and diff check passed | Checks the first merged main contracts locally, not hosted Linux or GitHub workflow validation |
...
Exhaustive completion-race interleavings, complete consumer CLI use, production
image publication, and Hex publication remain unverified. Hosted Linux/macOS
qualification and exact cross-host artifact parity passed on the rebased code
head. Mermaid rendering on GitHub has not yet been checked.
...
| Second rebase recheck | Astra xhigh independently confirmed all seven PR patches are unchanged against main `82446046`, reviewed plan and historical Python blobs are identical, and the new mainline lifecycle fixture is retained. The reviewer checked the pre-rebase green CI/parity evidence and revised counts, with no findings on 2026-09-23. New-base hosted qualification remains required. |
| Final hosted-evidence recheck | Astra xhigh independently verified CI run `35844118461` on the rebased code head passed native Linux/macOS and exact artifact parity, and image run `35844118520` passed PR qualification while publication/promotion remained skipped. The only later change is this record update. No findings on 2026-09-23. |
