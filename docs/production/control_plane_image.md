...
For `0.5.0-rc.10`, tag the already-qualified `main` commit:

```bash
git switch main
git pull --ff-only origin main
test "$(scripts/release_metadata.sh | sed -n 's/^FAVN_CONTROL_PLANE_VERSION=//p')" = "0.5.0-rc.10"
...
git tag -s v0.5.0-rc.10 -m "Favn 0.5.0-rc.10"
git push origin v0.5.0-rc.10
...
`ghcr.io/eirhop/favn-control-plane:v0.5.0-rc.10` lookup tag. Deploy the digest
