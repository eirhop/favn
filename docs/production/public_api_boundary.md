...
Add `favn_duckdb_adbc` when the project needs the preferred DuckDB execution
path:
...
`favn_duckdb` remains available as the legacy `duckdbex` plugin path while Favn
is still in private development.
