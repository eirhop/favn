...
check error; it does not make the asset retryable. Failed rollback, uncertain
commit, or a missing result retains the write hold.

For claims incorrectly held by the former retryability classification, use the
held-write procedure above with `verified_no_effect` only after establishing
that the original runner/backend are stopped and authoritative transaction
records prove no effect. A before-materialize label, contract error, or empty
table alone is insufficient. The audited resolution releases the hold without
rewriting history; submit corrected work afterward through the normal API.
