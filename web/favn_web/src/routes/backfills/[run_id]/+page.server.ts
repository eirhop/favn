...
export const load: PageServerLoad = async (event) => {
	const { locals, cookies, params, url } = event;
	const session = await requireProtectedPageSession(event);
	const windowSearch = new URLSearchParams(url.searchParams);
	if (!windowSearch.has('limit')) windowSearch.set('limit', '50');
	if (!windowSearch.has('offset')) windowSearch.set('offset', '0');

	const [runResponse, windowsResponse, activeManifestResponse] = await Promise.all([
		orchestratorGetRun(session, params.run_id),
		orchestratorListBackfillWindows(session, params.run_id, windowSearch),
		orchestratorGetActiveManifest(session)
	]);

	if (
		runResponse.status === 401 ||
		windowsResponse.status === 401 ||
		activeManifestResponse.status === 401
	) {
		clearLocalSession({ locals, cookies });
		throw redirect(303, '/login');
	}
	if (!runResponse.ok) throw error(runResponse.status, 'Failed to load backfill parent run');

	const runPayload = await readJsonOr(runResponse, {});
	const windowsPayload = windowsResponse.ok ? await readJsonOr(windowsResponse, []) : [];
	...
	return {
		...
		windowsPage: normalizeBackfillWindows(windowsPayload),
		loadError: windowsResponse.ok ? null : `HTTP ${windowsResponse.status}`
	};
};
