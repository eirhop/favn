defmodule FavnStoragePostgres.StorageV2.Migrations do
  ...
  @migrations [
    ...
    {20_260_924_000_000, FavnStoragePostgres.Migrations.AddGenerationPublicationV2},
    {20_260_924_010_000, FavnStoragePostgres.Migrations.RetireInitialGenerationRecoveryV2}
  ]
  @required_tables ~w(
    retention_floors
    schema_migrations
  ...
  @critical_indexes ~w(
    asset_evidence_bindings_initial_manifest_id_index
    ...
    runs_root_submission_history_idx
    ...
    runner_tasks_session_attribution_idx
    runner_tasks_busy_window_idx
    runner_sessions_open_instance_idx
    runner_sessions_open_boot_idx
    runner_sessions_recent_idx
    runner_sessions_prune_idx
    ...
    target_statuses_logical_latest_idx
  ...
  @required_columns %{
    ...
    "runner_sessions" =>
      ~w(session_id runner_instance_id runner_boot_id session_generation control_plane_boot_id runner_pool required_runner_release_id beam_node protocol_version lifecycle_mode registered_at ended_at end_reason busy_at_exit interrupted_task_workspace_id interrupted_task_id inserted_at updated_at),
    "execution_packages" =>
    ...
    "materialization_claims" =>
      ~w(workspace_id claim_key purpose deployment_id target_kind target_id target_generation_id evidence_generation_id partition_key run_id operation_id claim_command_id claim_request_hash effect_state effect_task_id effect_assignment_generation effect_started_at effect_resolution owner_id fencing_token last_renewal_id last_finish_command_id finish_hash status expires_at completed_at result error version inserted_at updated_at),
    "materializations" =>
    ...
    "run_ownerships" =>
      ~w(recovery_disposition recovery_attempts claim_purpose next_recovery_at attention_revision last_renewed_at diagnosis_reason workspace_id run_id owner_id fencing_token claim_command_id last_renewal_id expires_at released_at updated_at),
    "run_execution_checkpoints" =>
      ~w(workspace_id run_id owner_id fencing_token checkpoint_version checkpoint_revision checkpoint_sequence stage attempt payload payload_hash updated_at),
    "run_plans" =>
    ...
    "schedule_activation_commands" =>
      ~w(workspace_id command_id request_hash result inserted_at updated_at),
    "schedule_cursors" =>
    ...
    "target_operation_locks" =>
      ~w(last_renewal_id workspace_id target_id operation_id operation_type fencing_token lease_owner lease_expires_at effect_state effect_task_id effect_assignment_generation effect_started_at effect_resolution version inserted_at updated_at),
    "workspace_deployment_targets" =>
      ~w(workspace_id deployment_id target_kind target_id selection_source customer_visible descriptor inserted_at),
    "workspace_deployments" =>
      ~w(retiring workspace_id deployment_id manifest_version_id configuration configuration_fingerprint target_catalog_fingerprint configuration_version deployed_by_actor_id inserted_at),
    "workspace_runtime_state" =>
    ...
    "workspace_provisioning_operations" =>
      ~w(operation_id workspace_id request_fingerprint actor_id username authentication_mode tenant_fingerprint object_fingerprint status inserted_at updated_at)
  }
  @identifier_constraint_tables ~w(
    ...
    run_ownerships run_execution_checkpoints schedule_cursors schedule_activations
  ...
  @critical_constraints ~w(
    ...
    runner_tasks_generation_precondition_bounded runner_tasks_status_valid runner_tasks_kind_valid runner_tasks_retry_class_valid
    ...
    runner_sessions_end_shape_valid
    runner_sessions_identity_valid
    ...
    run_ownerships_recovery_valid run_ownerships_fence_valid run_execution_checkpoints_values_valid schedule_activations_values_valid
    ...
    run_ownerships_run_fk run_execution_checkpoints_run_fk runner_tasks_workspace_fkey
  ...
  @expected_definition_fingerprint "2571f52f0d53c00155ca94ee1a9109ef5a913d674e69b6d32c27fb85f79f02db"
    ...
    with {:ok, columns} <-
         ...
         {:ok, triggers} <-
           query_rows(
             repo,
             """
             SELECT c.relname,t.tgname,t.tgenabled,pg_catalog.pg_get_triggerdef(t.oid,true),pg_catalog.pg_get_functiondef(t.tgfoid)
      ...
      fingerprint =
        %{columns: columns, constraints: constraints, indexes: indexes, triggers: triggers}
