defmodule FavnStoragePostgres.StorageV2.CoreAuthorityTest do
  ...
  alias FavnOrchestrator.TestSupport.ManagedRun, as: RunServer
  ...
  defmodule CompletedLostReplyStore do
    ...
    def admit(command) do
      with {:ok, task} <- FavnStoragePostgres.RunnerTasks.Store.admit(command) do
        gate = Application.fetch_env!(:favn_storage_postgres, :completed_lost_reply_counter)

        count =
          Agent.get_and_update(gate, fn counts ->
            run_id = command.enqueue.run_id
            count = Map.get(counts, run_id, 0) + 1
            {count, Map.put(counts, run_id, count)}
          end)
        ...
        if count == 2 do
          ...
          receive do
            :release_lost_reply ->
              {:error,
               FavnOrchestrator.Persistence.Error.new(:unavailable, "reply lost after execution",
                 retryable?: true
               )}
          end
        else
  ...
  defmodule HeldTargetAdmissionReplyStore do
    @behaviour FavnOrchestrator.Persistence.RunnerTaskStore
    for {operation, arity} <-
          FavnOrchestrator.Persistence.RunnerTaskStore.behaviour_info(:callbacks) -- [admit: 1] do
      args = Macro.generate_arguments(arity, __MODULE__)
    ...
    @impl true
    def admit(command) do
      result = FavnStoragePostgres.RunnerTasks.Store.admit(command)

      gate = Application.fetch_env!(:favn_storage_postgres, :held_target_reply_gate)

      first? =
        command.target_lock && match?({:ok, %{status: :admitted}}, result) &&
          Agent.get_and_update(gate, fn seen ->
            {not MapSet.member?(seen, command.enqueue.run_id),
             MapSet.put(seen, command.enqueue.run_id)}
          end)

      if command.target_lock && match?({:ok, %{status: :admitted}}, result) && first? do
        send(
          Application.fetch_env!(:favn_storage_postgres, :held_target_reply_observer),
          {:target_admission_committed, self(), command.enqueue.task_id}
        )

        receive do
          :release_target_reply -> :ok
        end
      end
    ...
    def admit(admission) do
      ...
      {action, test_pid} =
        Agent.get_and_update(gate, fn state ->
          ...
          task_id = command.event.data.runner_task_id
          seen = Map.get(state.seen, run_id, [])

          {ordinal, seen} =
            case Enum.find_index(seen, &(&1 == task_id)) do
              nil -> {length(seen) + 1, seen ++ [task_id]}
              index -> {index + 1, seen}
            end
          ...
          action =
            cond do
              MapSet.member?(state.reply_losses, key) -> :reply_loss
              MapSet.member?(state.gates, key) -> :gate
              true -> :delegate
            end

          state = %{
            state
            | seen: Map.put(state.seen, run_id, seen),
              gates: MapSet.delete(state.gates, key),
              reply_losses: MapSet.delete(state.reply_losses, key)
          }

          {{action, state.test_pid}, state}
        end)

      if action == :gate do
        ref = make_ref()
        send(test_pid, {:step_started_gated, self(), ref, command})

        receive do
          {:release_step_started, ^ref} -> :ok
        after
          5_000 -> exit(:step_started_gate_timeout)
        end
      end
        ...
        case action do
          :reply_loss ->
            ...
            {:error,
             FavnOrchestrator.Persistence.Error.new(
               :unavailable,
               "injected reply loss after durable commit",
               retryable?: true
             )}
  ...
  defmodule LifecycleGate do
    def call(operation, command, delegate) do
      gate = Application.fetch_env!(:favn_storage_postgres, :lifecycle_gate)

      {pause?, owner} =
        Agent.get_and_update(gate, fn state ->
          seen = Map.get(state.seen, operation, [])
          identity = Map.get(command, :command_id)
          seen = if identity in seen, do: seen, else: seen ++ [identity]
          key = {operation, Enum.find_index(seen, &(&1 == identity)) + 1}
          pause? = MapSet.member?(state.gates, key)

          {{pause?, state.owner},
           %{
             state
             | seen: Map.put(state.seen, operation, seen),
               gates: MapSet.delete(state.gates, key)
           }}
        end)
      ...
      if pause? do
        ref = make_ref()
        send(owner, {:lifecycle_gated, operation, self(), ref, command})

        receive do
          {:release_lifecycle, ^ref} -> :ok
        after
          5_000 -> exit(:lifecycle_gate_timeout)
        end
      end

      result = delegate.()
      send(owner, {:lifecycle_result, operation, command, result})
      result
    end
  end

  defmodule LifecycleRunStore do
    ...
    for {operation, arity} <-
      ...
      @impl true
      def unquote(operation)(unquote_splicing(args)),
        do: apply(FavnStoragePostgres.Runs.Store, unquote(operation), unquote(args))
    end

    @impl true
    def commit_transition(%{event: %{event_type: :step_queued}} = command),
      do: LifecycleGate.call(:step_queued, command, fn -> RunStore.commit_transition(command) end)
  ...
  defmodule LifecycleCircuitStore do
    ...
    for {operation, arity} <-
          FavnOrchestrator.Persistence.ResourceCircuitStore.behaviour_info(:callbacks) --
            [record_outcomes: 1] do
      args = Macro.generate_arguments(arity, __MODULE__)
      @impl true
      def unquote(operation)(unquote_splicing(args)),
        do: apply(FavnStoragePostgres.ResourceCircuits.Store, unquote(operation), unquote(args))
    end

    @impl true
    def record_outcomes(command),
      do:
        LifecycleGate.call(:resource_outcomes, command, fn ->
          FavnStoragePostgres.ResourceCircuits.Store.record_outcomes(command)
        end)
  end

  defmodule UnavailableCancellationTaskStore do
    @behaviour FavnOrchestrator.Persistence.RunnerTaskStore
    ...
    def request_cancellation(_command) do
      {:error,
       FavnOrchestrator.Persistence.Error.new(:unavailable, "injected cancellation failure",
         retryable?: true
       )}
    end
  end

  defmodule ObservedRunOwnershipStore do
    @behaviour FavnOrchestrator.Persistence.RunOwnershipStore

    @delegate FavnStoragePostgres.RunOwnership.Store

    for {operation, arity} <-
          FavnOrchestrator.Persistence.RunOwnershipStore.behaviour_info(:callbacks) --
            [renew_run: 1] do
      arguments = Macro.generate_arguments(arity, __MODULE__)
    ...
    def renew_run(command) do
      result = @delegate.renew_run(command)
      gate = Application.fetch_env!(:favn_storage_postgres, :step_started_gate)
      test_pid = Agent.get(gate, & &1.test_pid)
      send(test_pid, {:ownership_renewal_result, command.run_id, command.renewal_id, result})
      result
    end
  end
  ...
  setup tags do
    ...
    url =
      if tags[:committed_lifecycle],
        do: FavnStoragePostgres.TestSupport.IsolatedDatabase.create!(url),
        else: url
    ...
    options = if tags[:committed_lifecycle], do: Keyword.delete(options, :pool), else: options
    Process.put(:committed_lifecycle, tags[:committed_lifecycle])
    Process.put(:test_database_url, url)
    start_supervised!({Repo, options})
    start_supervised!({FavnStoragePostgres.RunLeaseRepo, Keyword.delete(options, :pool)})
    :ok = Migrations.migrate!(Repo)

    unless tags[:committed_lifecycle] do
  ...
  test "three concurrent runs retain authority during a 32 second coordinator stall under scheduler pressure",
       fixture do
    fixtures = [
      fixture,
    ...
    runs =
      Enum.map(fixtures, fn f ->
        {command, run} = pipeline_run_command(f)
        ...
        run =
          %{run | plan: plan, plan_hash: RunState.plan_hash(plan)}
          |> RunState.with_snapshot_hash()

        assert {:ok, _} = RunStore.create_run(%{command | run: run})
        assert {:ok, pid} = RunServer.start_link(%{run_state: run})
        monitor = Process.monitor(pid)
        assert [_] = await_runner_task_ids!(f.workspace_id, run.id, 1)
        assert {:ok, task} = claim_asset_task(f, "cpu-runner")
        assert :ok = start_runner_task(task)
        await_runner_task_waiter!(task)

        %{rows: [[generation, expiry]]} =
          SQL.query!(
            Repo,
            "SELECT fencing_token,expires_at FROM favn_control.run_ownerships WHERE workspace_id=$1 AND run_id=$2",
            [f.workspace_id, run.id]
          )
    ...
    burners = for _ <- 1..2, do: spawn(fn -> burn_cpu() end)

    try do
      finishers =
        Enum.map(runs, fn r ->
          Task.async(fn ->
            receive do: (:finish -> complete_asset_task(r.task, r.task.payload, false))
          end)
        end)

      Enum.each(finishers, &send(&1.pid, :finish))
      Enum.each(finishers, &assert(Task.await(&1, 5_000) == :ok))
      Process.sleep(32_000)

      for r <- runs do
        assert Process.alive?(r.pid)

        assert %{rows: [[generation, renewed, headroom]]} =
                 SQL.query!(
                   Repo,
                   "SELECT fencing_token,expires_at,EXTRACT(EPOCH FROM (expires_at-clock_timestamp())) FROM favn_control.run_ownerships WHERE workspace_id=$1 AND run_id=$2",
                   [r.fixture.workspace_id, r.run.id]
                 )

        assert generation == r.generation
        assert DateTime.compare(renewed, r.expiry) == :gt
        assert Decimal.compare(headroom, Decimal.new(20)) == :gt
        :ok = :sys.resume(r.pid)
      end

      for %{pid: pid, monitor: monitor} = r <- runs do
        assert_receive {:DOWN, ^monitor, :process, ^pid, :normal}, 10_000
        assert length(runner_task_ids(r.fixture.workspace_id, r.run.id)) == 1
        assert run_event_count(r.fixture.workspace_id, r.run.id, "step_settled") == 1
      end
    after
      Enum.each(burners, &Process.exit(&1, :kill))
      :erlang.system_flag(:schedulers_online, schedulers)
      for r <- runs, Process.alive?(r.pid), do: :sys.resume(r.pid)
    end
  end

  defp burn_cpu do
    :crypto.hash(:sha256, :binary.copy(<<42>>, 16_384))
    burn_cpu()
  end
  ...
  for mode <- [:renew, :expired, :malformed, :missing_lock] do
    ...
    test "combined target admission with held reply: #{mode}", %{target_reply_mode: mode} do
      fixture = continuation_distinct_target_fixture!()
      ...
      gate = start_supervised!({Agent, fn -> MapSet.new() end})
      Application.put_env(:favn_storage_postgres, :held_target_reply_gate, gate)
      on_exit(fn -> Application.delete_env(:favn_storage_postgres, :held_target_reply_gate) end)
      Application.put_env(:favn_storage_postgres, :held_target_reply_observer, self())

      on_exit(fn ->
        Application.delete_env(:favn_storage_postgres, :held_target_reply_observer)
      end)
      ...
      {:ok, runtime} =
        Runtime.start_link(%Runtime{
          backend: Backend,
          options: [],
          stores: %{Backend.stores() | runner_tasks: HeldTargetAdmissionReplyStore}
        })
      ...
      Process.unlink(pid)
      monitor = Process.monitor(pid)
      assert_receive {:target_admission_committed, helper, task_id}, 5_000
      assert {:ok, task} = RunnerTasks.fetch(fixture.workspace_id, task_id)
      lock = task.orchestration_context.materialization_claim.target_operation_lock
      assert lock != nil

      %{rows: [[fence, expires]]} =
        SQL.query!(
          Repo,
          "SELECT fencing_token,lease_expires_at FROM favn_control.target_operation_locks WHERE workspace_id=$1 AND target_id=$2",
          [fixture.workspace_id, lock.target_id]
        )

      await_target_renewed!(fixture.workspace_id, lock.target_id, fence, expires, 1_000)

      entry =
        :sys.get_state(FavnOrchestrator.RunManager).lifecycles
        |> Map.values()
        |> Enum.find(&(&1.key == {fixture.workspace_id, run.id}))

      watch = :sys.get_state(entry.maintenance).watches[task_id]
      assert watch.acquisition_deadline == nil
      ...
      if mode == :renew do
        ...
        send(helper, :release_target_reply)
        assert [_, _, _] = await_runner_task_ids!(fixture.workspace_id, run.id, 3)

        for n <- 1..3 do
          assert {:ok, claimed} = claim_asset_task(fixture, "held-reply-#{n}")
          assert :ok = start_runner_task(claimed)
          assert :ok = complete_asset_task(claimed, claimed.payload, false)
        end
        ...
        original = runner_task_ids(fixture.workspace_id, run.id)
        ...
        if mode == :expired do
          ...
          SQL.query!(
            Repo,
            "UPDATE favn_control.runner_tasks SET orchestration_context=$3 WHERE workspace_id=$1 AND task_id=$2",
            [fixture.workspace_id, task_id, malformed]
          )
        end

        send(entry.maintenance, :renew)
        assert_receive {:DOWN, ^monitor, :process, ^pid, _}, 6_000
        await_released_ownership!(fixture.workspace_id, run.id)

        SQL.query!(
          Repo,
          "UPDATE favn_control.run_ownerships SET next_recovery_at=clock_timestamp()-interval '1 second' WHERE workspace_id=$1 AND run_id=$2",
        ...
        assert runner_task_ids(fixture.workspace_id, run.id) == original

        assert %{rows: [[^fence]]} =
                 SQL.query!(
                   Repo,
                   "SELECT fencing_token FROM favn_control.target_operation_locks WHERE workspace_id=$1 AND target_id=$2",
                   [fixture.workspace_id, lock.target_id]
                 )
      end
    end
  ...
  defp await_target_renewed!(workspace, target, fence, expires, attempts) do
    assert attempts > 0, "target was not independently renewed within ten seconds"

    %{rows: [[actual_fence, renewed]]} =
      SQL.query!(
        Repo,
        "SELECT fencing_token,lease_expires_at FROM favn_control.target_operation_locks WHERE workspace_id=$1 AND target_id=$2",
        [workspace, target]
    ...
    if DateTime.compare(renewed, expires) != :gt do
      receive do
      after
        10 -> :ok
      end

      await_target_renewed!(workspace, target, fence, expires, attempts - 1)
    end
  end

  defp await_failed_cleanup!(workspace, run, attempts) do
    assert attempts > 0, "recovery did not persist failure and cleanup intent"

    %{rows: [[status, cleanup]]} =
      SQL.query!(
        Repo,
        "SELECT status, snapshot->'metadata'->'failure_cleanup' FROM favn_control.runs WHERE workspace_id=$1 AND run_id=$2",
        [workspace, run]
      )

    if status != "error" or not is_map(cleanup) do
      Process.sleep(10)
      await_failed_cleanup!(workspace, run, attempts - 1)
    else
      assert cleanup["state"] in ["pending", "attention"]
    end
  end
  ...
  test "recovery drains terminal siblings with an expired combined-window lock and an unknown write",
    ...
    assert_received {:recovered_ownership_renewal, renewal_worker, :ok}
    refute renewal_worker == recovered
  ...
  test "keyed asset attempt detail is scoped by workspace and exact run", fixture do
    other = provision_deploy_fixture(fixture.version)
    shared_run_id = "shared-detail-#{System.unique_integer([:positive])}"
    asset_step_id = "shared-step"

    {command, run} = create_run_command(fixture, shared_run_id)
    {other_command, other_run} = create_run_command(other, shared_run_id)
    {sibling_command, sibling_run} = create_run_command(fixture)

    assert {:ok, _created} = RunStore.create_run(command)
    assert {:ok, _created} = RunStore.create_run(other_command)
    assert {:ok, _created} = RunStore.create_run(sibling_command)

    for {workspace_id, exact_run_id, marker} <- [
          {fixture.workspace_id, run.id, "selected"},
  ...
  test "fences run ownership", fixture do
    ...
    assert {:ok, replayed_claim} = RunOwnershipStore.claim_run(claim)
    assert replayed_claim.expires_at == ownership.expires_at
    ...
    assert {:ok, replayed_renewal} = RunOwnershipStore.renew_run(renewal)
    assert replayed_renewal.expires_at == renewed.expires_at
  ...
  for disposition <- [:complete, :cancel] do
    @tag disposition: disposition
    @tag committed_lifecycle: true
    test "sequential committed enqueue reply loss reconciles before #{disposition}", fixture do
      {command, original} = create_run_command(fixture)
      ...
      {:ok, runtime} =
        Runtime.start_link(%Runtime{
          backend: Backend,
          options: [],
          stores: %{Backend.stores() | runner_tasks: LostEnqueueReplyStore}
        })
      ...
      case fixture.disposition do
        :complete ->
          assert {:ok, task} = claim_asset_task(fixture, "sequential-reply-loss")
      ...
      assert_receive {:DOWN, ^monitor, :process, ^pid, :normal}, 5_000
      assert {:ok, finished} = get_run(fixture, run.id)
      assert finished.status == if(fixture.disposition == :complete, do: :ok, else: :cancelled)
      assert [^task_id] = runner_task_ids(fixture.workspace_id, run.id)
      expected = if fixture.disposition == :complete, do: :succeeded, else: :cancelled
      assert ^expected = await_runner_task_status!(fixture.workspace_id, task_id, expected)
    end
  end

  @tag committed_lifecycle: true
  test "already-completed uncertain enqueue drains its success and earlier sibling", fixture do
    {run, _keys} = create_continuation_pipeline_run!(fixture, 3)
    counter = start_supervised!({Agent, fn -> %{} end})
    Application.put_env(:favn_storage_postgres, :completed_lost_reply_counter, counter)

    on_exit(fn ->
      Application.delete_env(:favn_storage_postgres, :completed_lost_reply_counter)
    end)

    Application.put_env(:favn_storage_postgres, :completed_lost_reply_owner, self())
    on_exit(fn -> Application.delete_env(:favn_storage_postgres, :completed_lost_reply_owner) end)

    {:ok, runtime} =
      Runtime.start_link(%Runtime{
        backend: Backend,
        options: [],
        stores: %{Backend.stores() | runner_tasks: CompletedLostReplyStore}
      })
    ...
    assert {:ok, pid} = RunServer.start_link(%{run_state: run, version: fixture.version})
    monitor = Process.monitor(pid)
    assert_receive {:saved_before_lost_reply, helper, uncertain_id}, 5_000
    ...
    {uncertain, sibling} =
      if first.task_id == uncertain_id do
        ...
        send(helper, :release_lost_reply)
    ...
    assert %{rows: [[0]]} =
             SQL.query!(
               Repo,
               "SELECT count(*) FROM favn_control.materialization_claims WHERE workspace_id=$1 AND run_id=$2 AND status <> 'succeeded'",
               [fixture.workspace_id, run.id]
             )

    assert :succeeded = await_runner_task_status!(fixture.workspace_id, uncertain_id, :succeeded)
  ...
  test "live history guards share a root while retirement remains exclusive", fixture do
    {command, run} = pipeline_run_command(fixture)
    assert {:ok, _} = RunStore.create_run(command)
    root = install_pipeline_history_probe_root!(fixture.workspace_id, run.id)
    holder = hold_pipeline_history_lock!(fixture.workspace_id, root, :shared)

    assert {:ok, :ok} =
             Repo.transaction(fn ->
               FavnStoragePostgres.Maintenance.History.guard!(fixture.workspace_id, run.id)

               assert FavnStoragePostgres.Maintenance.History.try_guard!(
                        fixture.workspace_id,
                        run.id
                      )
               ...
               assert %{rows: [[false]]} =
                        SQL.query!(
                          Repo,
                          "SELECT pg_try_advisory_xact_lock(hashtextextended(jsonb_build_array('execution_history',$1::text,$2::text)::text,0))",
    ...
    root = install_pipeline_history_probe_root!(fixture.workspace_id, run.id)
    holder = hold_pipeline_history_lock!(fixture.workspace_id, root)
  ...
  @tag committed_lifecycle: true
  test "completion bookkeeping and admission queue contention recover in one pipeline", fixture do
    owner = self()
    ...
    {:ok, gate} =
      Agent.start_link(fn ->
        %{
          owner: owner,
          seen: %{},
          gates: MapSet.new([{:resource_outcomes, 1}, {:step_queued, 2}])
        }
      end)
    ...
    {:ok, runtime} =
      Runtime.start_link(%Runtime{
        ...
        stores: %{
          Backend.stores()
          | runs: LifecycleRunStore,
            resource_circuits: LifecycleCircuitStore
        }
      })
    ...
    on_exit(fn ->
      ...
      Application.delete_env(:favn_storage_postgres, :lifecycle_gate)
    end)
    ...
    install_execution_capacity_scope!(fixture, :default, policies)
    asset = Enum.find(fixture.version.manifest.assets, &(&1.ref == {MyApp.Asset, :asset}))
    ...
    plan = %{
      plan
      | nodes:
          Map.new(plan.nodes, fn {key, node} ->
            {key, %{node | execution_pool: :default}}
          end)
    }
    ...
    run =
      RunState.new(
        ...
        metadata: %{
          ...
          execution_pool_policy: policies
        }
      )
    ...
    assert {:ok, first} = claim_asset_task(fixture, "lifecycle-contention-1")
    assert :ok = start_runner_task(first)
    await_runner_task_waiter!(first)

    :ok =
      complete_asset_task(first, first.payload, false, %{
        manifest_uri: "fixture://landing",
        pages_written: 1
      })

    for operation <- [:resource_outcomes, :step_queued] do
      assert_receive {:lifecycle_gated, ^operation, worker, ref, frozen_command}, 5_000
      root = install_pipeline_history_probe_root!(fixture.workspace_id, run.id)
      holder = hold_pipeline_history_lock!(fixture.workspace_id, root)
      send(worker, {:release_lifecycle, ref})

      assert_receive {:lifecycle_result, ^operation, ^frozen_command,
                      {:error,
                       %FavnOrchestrator.Persistence.Error{
                         retryable?: true,
                         details: %{reason_code: "execution_history_owner_busy"}
      ...
      assert Map.get(state, :execution_persist_pending),
             "#{operation} must pause rather than fail the run"

      assert state.execution_state.terminal_failure == nil
      restore_pipeline_history_root!(fixture.workspace_id, run.id, root)
      release_pipeline_history_lock!(holder)
      assert_receive {:lifecycle_result, ^operation, ^frozen_command, {:ok, _}}, 5_000
    end

    writes =
      Enum.map(2..5, fn index ->
        assert ids = await_runner_task_ids!(fixture.workspace_id, run.id, index)
        assert length(ids) == index
        assert {:ok, task} = claim_asset_task(fixture, "lifecycle-contention-#{index}")
        assert :ok = start_runner_task(task)
        await_runner_task_waiter!(task)

        if index == 3 do
          # A new stage receives its own admission budget, even if the previous
          # stage used its entire budget while its already admitted task ran.
          :sys.replace_state(pid, fn state ->
            put_in(
              state.execution_state.stage_admission_deadline_ms,
    ...
    assert Enum.sort([Favn.Contracts.RunnerWork.node_key(first.payload) | writes]) ==
             keys |> Map.values() |> Enum.sort()

    assert_receive {:DOWN, ^monitor, :process, ^pid, :normal}, 5_000
    assert {:ok, finished} = get_run(fixture, run.id)
    assert finished.status == :ok
    assert length(finished.result.node_results) == 5
    ...
    assert %{rows: [[0]]} =
             SQL.query!(
               Repo,
               "SELECT count(*) FROM favn_control.runner_tasks WHERE workspace_id=$1 AND run_id=$2 AND status <> 'succeeded'",
               [fixture.workspace_id, run.id]
             )
    ...
    assert %{rows: [[0]]} =
             SQL.query!(
               Repo,
               "SELECT count(*) FROM favn_control.materialization_claims WHERE workspace_id=$1 AND run_id=$2 AND status='claimed'",
               [fixture.workspace_id, run.id]
             )
  end

  @tag committed_lifecycle: true
  @tag lifecycle_contention_fix: true
  test "real history contention recovers admission, reply loss, and cancellation", fixture do
    install_gated_run_store!()
    start_pipeline_runtime!()
    ...
    assert {:ok, pid} = RunServer.start_link(%{run_state: run, version: fixture.version})
    monitor = Process.monitor(pid)
    assert_receive {:step_started_gated, helper, gate_ref, gated_command}, 5_000
    ...
    send(helper, {:release_step_started, gate_ref})

    assert_receive {:step_started_result, ^run_id, ^gated_command,
                    {:error,
                     %FavnOrchestrator.Persistence.Error{
                       details: %{reason_code: "execution_history_owner_busy"},
                       retryable?: true
    ...
    paused = await_execution_retry!(pid)
    ...
    assert_receive {:ownership_renewal_result, ^run_id, renewal_id,
                    {:error,
                     %FavnOrchestrator.Persistence.Error{
                       details: %{reason_code: "execution_history_owner_busy"},
                       retryable?: true
    ...
    restore_pipeline_history_root!(fixture.workspace_id, run.id, probe_root)
    release_pipeline_history_lock!(holder)

    assert_receive {:ownership_renewal_result, ^run_id, ^renewal_id, {:ok, _renewed}}, 5_000

    task_ids = await_runner_task_ids!(fixture.workspace_id, run.id, 3)
    assert saved_task_id in task_ids
    assert length(Enum.uniq(task_ids)) == 3

    Enum.each(1..3, fn index ->
      assert {:ok, task} = claim_asset_task(fixture, "contended-batch-#{index}")
      assert :ok = start_runner_task(task)
      await_runner_task_waiter!(task)
      :ok = complete_asset_task(task, task.payload, false)
    end)
    ...
    assert finished.status == :ok
    refute Map.has_key?(finished.metadata, :cancel_outcomes)
    assert run_event_count(fixture.workspace_id, run.id, "step_started") == 3
    ...
    assert {:ok, first} = claim_asset_task(fixture, "contended-refill-1")
    assert :ok = start_runner_task(first)
    await_runner_task_waiter!(first)
    :ok = complete_asset_task(first, first.payload, false)
    ...
    Enum.each(2..3, fn index ->
      assert task_ids = await_runner_task_ids!(fixture.workspace_id, run.id, index)
      assert length(task_ids) == index
      assert {:ok, task} = claim_asset_task(fixture, "contended-refill-#{index}")
      assert :ok = start_runner_task(task)
      await_runner_task_waiter!(task)
      :ok = complete_asset_task(task, task.payload, false)
    end)
    ...
    {run, _keys} = create_continuation_pipeline_run!(fixture, 1)
    run_id = run.id
    lose_step_started_reply!(run.id, 1)

    assert {:ok, pid} = RunServer.start_link(%{run_state: run, version: fixture.version})
    monitor = Process.monitor(pid)

    assert_receive {:step_started_commit_reply_lost, first_command, {:ok, %{replayed?: false}}},
                   5_000

    assert first_command.run.id == run.id

    assert_receive {:step_started_result, ^run_id, ^first_command,
                    {:error, %{kind: :unavailable, retryable?: true}}},
                   5_000

    assert_receive {:step_started_result, ^run_id, replay_command, {:ok, %{replayed?: true}}},
                   5_000
    ...
    Enum.each(1..3, fn index ->
      assert task_ids = await_runner_task_ids!(fixture.workspace_id, run.id, index)
      assert length(task_ids) == index
      assert {:ok, task} = claim_asset_task(fixture, "reply-loss-#{index}")
      assert :ok = start_runner_task(task)
      await_runner_task_waiter!(task)
      :ok = complete_asset_task(task, task.payload, false)
    end)
    ...
    monitor = Process.monitor(pid)
    assert_receive {:step_started_gated, helper, gate_ref, gated_command}, 5_000
    local_task_id = gated_command.event.data.runner_task_id
    assert [saved_sibling_id] = await_runner_task_ids!(fixture.workspace_id, run.id, 1)
    ...
    assert :ok =
             RunStore.request_operation_cancellation(%RequestRunCancellation{
               workspace_context: fixture.workspace_context,
               command_id: "contended-cancel:" <> run.id,
               run_id: run.id,
               reason: reason,
               occurred_at: DateTime.utc_now()
             })
    ...
    cancelling = await_tracked_cancellation!(pid, saved_sibling_id)
    assert cancelling.execution_state.paused_admission == nil
    assert Map.has_key?(cancelling.execution_state.awaits, saved_sibling_id)
    refute Map.has_key?(cancelling.execution_state.awaits, local_task_id)

    now = DateTime.utc_now()

    assert {:ok, %{status: :cancelled} = cancelled_task} =
             FavnStoragePostgres.RunnerTasks.Store.request_cancellation(
               %FavnOrchestrator.Persistence.Commands.RequestRunnerTaskCancellation{
                 workspace_context: fixture.workspace_context,
                 command_id: "persist-cancel:" <> saved_sibling_id,
                 task_id: saved_sibling_id,
                 reason: reason,
                 issued_at: now,
                 occurred_at: now
               }
    ...
    RunnerTaskResultRouter.notify(cancelled_task)

    assert_receive {:DOWN, ^monitor, :process, ^pid, :normal}, 5_000
    assert {:ok, cancelled} = get_run(fixture, run.id)
    assert cancelled.status == :cancelled
    assert Map.get(cancelled.metadata, :cancel_requested, cancelled.metadata["cancel_requested"])
    assert [^saved_sibling_id] = runner_task_ids(fixture.workspace_id, run.id)

    assert {:ok, persisted_task} =
             RunnerTaskStore.get(%FavnOrchestrator.Persistence.Queries.GetRunnerTask{
               workspace_context: fixture.workspace_context,
               task_id: saved_sibling_id
             })
  ...
  test "terminal failure during refill preserves a sibling admitted by an earlier batch",
    ...
    blocker = %{
      admit_command(fixture, second_run.id, "terminal-refill-blocker")
      | requests: [%CapacityRequest{scope_id: global_scope.scope_id}]
    }

    assert {:ok, %{status: :admitted, lease: blocking_lease}} =
             AdmissionStore.admit(blocker)
    ...
    await_run_event!(fixture.workspace_id, run.id, "step_failed", 2)

    assert :assigned ==
             await_runner_task_status!(fixture.workspace_id, task_id, :assigned)
  ...
  test "pipeline recovery refills deferred admission without duplicate tasks or leases",
    ...
    assert_receive {:first_recovery_refill_task_committed, helper}, 5_000
    refute helper == first_pid
    assert [_task_id] = runner_task_ids(fixture.workspace_id, run.id)
    assert active_execution_lease_count(fixture.workspace_id, run.id) == 1
  ...
  test "pipeline recovery adopts and releases the active lease at max concurrency one",
    ...
    Process.unlink(first_pid)
    monitor = Process.monitor(first_pid)
    assert_receive {:first_task_committed, helper}, 5_000
    refute helper == first_pid
    assert [_task_id] = runner_task_ids(fixture.workspace_id, run.id)
    assert active_execution_lease_count(fixture.workspace_id, run.id) == 1
    :telemetry.detach(handler)
  ...
  @tag committed_lifecycle: true
  @tag lifecycle_cancellation_fix: true
  ...
  defp await_cancelled_run!(fixture, run_id, remaining \\ 500)
  ...
  test "terminal task recovery frees the crashed owner's capacity before admitting its sibling",
    ...
    assert {:ok, first} = RunServer.start_link(%{run_state: run, version: fixture.version})
    assert_receive {:recovery_event_committed, helper, "step_started"}, 5_000
    refute helper == first
    :telemetry.detach(handler)
    assert [_] = await_runner_task_ids!(fixture.workspace_id, run.id, 1)
  ...
  test "a lost materialization finish reply recovers the accepted result without a second task",
    ...
    assert :ok =
             FavnOrchestrator.resume_run_recovery(
               fixture.workspace_context,
               run.id,
               saved.metadata["recovery_attention"]["revision"]
             )
  ...
  test "two crashes across outcome and settlement receipts preserve successful work", fixture do
    ...
    assert_receive {:recovery_event_committed, helper, "step_finished"}, 5_000
    refute helper == first
    :telemetry.detach(handler)
    kill_run_owner!(fixture, run.id, first)
    ...
    assert_receive {:recovery_event_committed, helper, "step_settled"}, 5_000
    refute helper == second
    :telemetry.detach(handler)
  ...
  for cause <- [:await_failure, :runner_timeout] do
    ...
    test "a crash after failed claim bookkeeping replays the exact accepted #{cause}",
      ...
      assert_receive {:failed_claim_committed, helper}, 5_000
      refute helper == first
      :telemetry.detach(handler)
  ...
  for boundary <- [:before_intent, :after_intent] do
    ...
    test "pipeline resumes every selected retry at attempt two after #{boundary}", fixture do
      ...
      assert Map.has_key?(tasks, keys.b)

      assert MapSet.subset?(
               MapSet.new(Map.keys(tasks)),
               MapSet.new([keys.a, keys.b, keys.c])
             )

      other_key = tasks |> Map.keys() |> Enum.find(&(&1 != keys.b))

      for {key, number} <- Enum.with_index([keys.b, other_key], 1) do
      ...
      third_key = Favn.Contracts.RunnerWork.node_key(third.payload)
      assert MapSet.new([other_key, third_key]) == MapSet.new([keys.a, keys.c])
      assert :ok = start_runner_task(third)
      await_runner_task_waiter!(third)
      assert :ok = complete_asset_task(third, third.payload, third_key == keys.a)
      await_run_event!(fixture.workspace_id, run.id, "step_settled", 3)

      assert_receive {:recovery_event_committed, helper, ^kind}, 5_000
      refute helper == first
      :telemetry.detach(handler)
  ...
  test "manager crash monitoring and failed restoration preserve queued work for the recovery sweep",
    ...
    assert {:ok, first} = RunServer.start_link(%{run_state: run, storage_ownership: initial})
    ...
    SQL.query!(
      Repo,
      "UPDATE favn_control.run_ownerships SET next_recovery_at=clock_timestamp() WHERE workspace_id=$1 AND run_id=$2",
      [fixture.workspace_id, run.id]
    )
    ...
    assert {:ok, recovered} =
             RunServer.start_link(%{run_state: run, storage_ownership: reclaimed})
  ...
  test "a stage-two crash preserves the first failure and continues its independent branch",
    ...
    assert_receive {:recovery_event_committed, helper, "step_intended"}, 5_000
    refute helper == first
    :telemetry.detach(handler)
    kill_run_owner!(fixture, run.id, first)
    assert {:ok, saved} = get_run(fixture, run.id)
  ...
  defp kill_run_owner!(fixture, run_id, pid) do
    ...
    await_released_ownership!(fixture.workspace_id, run_id)
  ...
  defp await_no_active_runs!(attempts) do
    assert attempts > 0, "run lifecycle did not finish draining"

    case FavnOrchestrator.RunManager.active_runs() do
      {:ok, []} ->
        :ok
      ...
      {:ok, _} ->
        ...
        await_no_active_runs!(attempts - 1)
    end
  end

  defp await_released_ownership!(workspace, run_id, remaining \\ 500)

  defp await_released_ownership!(_, _, 0),
    do: flunk("ownership was not released after lifecycle shutdown")

  defp await_released_ownership!(workspace, run_id, remaining) do
    case SQL.query!(
           Repo,
           "SELECT released_at IS NOT NULL FROM favn_control.run_ownerships WHERE workspace_id=$1 AND run_id=$2",
           [workspace, run_id]
         ) do
      %{rows: [[true]]} ->
        :ok
      ...
      _ ->
        Process.sleep(10)
        await_released_ownership!(workspace, run_id, remaining - 1)
    end
  end
  ...
  defmodule ContractSibling do
    def asset(context) do
      Ecto.Adapters.SQL.query!(
        ...
        "INSERT INTO public.issue736_effects(run_id) VALUES ($1)",
        [context.run_id]
      )
  ...
  for outcome <- [:check_rollback, :rejected_commit] do
    ...
    test "#{outcome} settles ownership and preserves independent work with node retries enabled" do
          ...
          %{
            issue736: %Favn.Connection.Resolved{
              ...
              config: %{
                ...
                database_url: Process.get(:test_database_url)
              }
            }
          },
          registry_name: FavnRunner.ConnectionRegistry
      ...
      SQL.query!(Repo, "CREATE TABLE public.issue736_effects(run_id text)", [])
        ...
        Map.new(Enum.zip(refs, keys), fn {asset_ref, key} ->
          ...
          {key,
           %{
             ...
             execution_pool: nil,
             action: :run,
             retry_policy: Favn.Retry.Policy.new!(max_attempts: 3, backoff: 0),
      ...
      assert {:ok, failed} = get_run(fixture, run.id)
      assert failed.status == :error
      assert failed.error["type"] == Atom.to_string(expected_error_type)

      assert Enum.any?(
               failed.result.node_results,
               &(&1.ref == dependent and &1.status == :blocked)
             )

      assert %{rows: [[1]]} =
               SQL.query!(Repo, "SELECT count(*) FROM public.issue736_effects WHERE run_id=$1", [
                 run.id
               ])
  ...
  defp await_run_event!(workspace, run_id, event, count, attempts \\ 100)
  defp await_run_event!(_workspace, _run_id, event, _count, 0), do: flunk("missing #{event}")

  defp await_run_event!(workspace, run_id, event, count, attempts) do
    if run_event_count(workspace, run_id, event) >= count do
      :ok
    else
      Process.sleep(20)
      await_run_event!(workspace, run_id, event, count, attempts - 1)
    end
  end

  defp hold_pipeline_history_lock!(workspace_id, root_run_id, mode \\ :exclusive) do
    lock_function =
      if mode == :shared, do: "pg_advisory_xact_lock_shared", else: "pg_advisory_xact_lock"

    owner = self()
    ref = make_ref()

    connection_options =
      Process.get(:test_database_url)
      ...
      Task.async(fn ->
        ...
        try do
          Postgrex.transaction(connection, fn transaction ->
            Postgrex.query!(
              transaction,
              "SELECT #{lock_function}(hashtextextended(jsonb_build_array('execution_history',$1::text,$2::text)::text,0))",
  ...
  defp claim_asset_task(fixture, runner_suffix \\ nil) do
    ...
    if result == {:ok, nil} and is_binary(runner_suffix) and
         Process.get({:task_claim_wait, runner_suffix}, 0) < 4_000 do
      Process.put(
        {:task_claim_wait, runner_suffix},
        Process.get({:task_claim_wait, runner_suffix}, 0) + 1
      )

      Process.sleep(10)
      claim_asset_task(fixture, runner_suffix)
    else
      Process.delete({:task_claim_wait, runner_suffix})
      result
    end
  end
  ...
  defp await_tracked_cancellation!(pid, task_id, remaining \\ 200)

  defp await_tracked_cancellation!(pid, task_id, 0) do
    state = :sys.get_state(pid)

    flunk(
      "saved sibling #{inspect(task_id)} was not tracked during cancellation: " <>
        inspect(%{
          paused_admission: state.execution_state.paused_admission,
          await_ids: Map.keys(state.execution_state.awaits),
          active_task_ids:
            FavnOrchestrator.RunServer.Execution.ActiveTaskSet.task_ids(
              state.execution_state.work_set
            ),
          run_status: state.execution_state.run.status,
          run_metadata: state.execution_state.run.metadata,
          persistence_pending: Map.get(state, :execution_persist_pending),
          renewal_pending: Map.get(state, :storage_renewal_pending)
        })
    )
  end

  defp await_tracked_cancellation!(pid, task_id, remaining) do
    state = :sys.get_state(pid)

    if state.execution_state.paused_admission == nil and
         Map.has_key?(state.execution_state.awaits, task_id) do
      state
    else
      Process.sleep(10)
      await_tracked_cancellation!(pid, task_id, remaining - 1)
    end
  end
  ...
  test "diagnostic log batches may name runs that have not been created", fixture do
    command = %AppendLogBatch{
      workspace_context: fixture.workspace_context,
      command_id: "precreation-logs",
      occurred_at: DateTime.utc_now(),
      batch_id: "precreation-logs",
      entries: [
        %LogEntry{
          run_id: "not-created-yet",
          source: "orchestrator",
          level: :error,
    ...
    assert entry.run_id == "not-created-yet"
    assert {:ok, [^entry]} = LogStore.append_batch(command)
  end
  ...
  test "projects ordered compact operator read models without group scans", fixture do
    ...
    assert [attempt] = flow.observed
    assert attempt.asset_step_id == "step:" <> run.id
    assert String.starts_with?(attempt.window_identity, "runtime:")
    assert flow.header.counts.total == 1
    assert flow.header.counts.succeeded == 0
    assert flow.header.counts.skipped == 0
    assert flow.header.counts.planned == 0
    assert flow.header.counts.running == 1
    refute flow.overflow?

    assert {:ok, event_summaries} =
             OperatorReadStore.list_run_event_summaries(%ListRunEventSummaries{
               workspace_context: fixture.workspace_context,
               run_id: run.id,
               limit: 10
    ...
    assert Enum.any?(event_summaries, &(&1.event_type == "step_running"))
    refute Enum.any?(event_summaries, &Map.has_key?(Map.from_struct(&1), :event))
    ...
    assert [%{status: :blocked}] = blocked_flow.observed
    assert blocked_flow.header.counts.failed == 1

    assert {:ok, blocked_attempt} =
             OperatorReadStore.get_run_asset_attempt(%GetRunAssetAttempt{
               workspace_context: fixture.workspace_context,
               run_id: run.id,
               asset_step_id: "step:" <> run.id
    ...
    assert blocked_attempt.error == "upstream_blocked"
    ...
    assert [%{status: :ok}] = completed_flow.observed
    assert completed_flow.header.counts.total == 1
    assert completed_flow.header.counts.completed == 1
    assert completed_flow.header.counts.succeeded == 1
    assert completed_flow.header.counts.skipped == 0
    assert completed_flow.header.counts.planned == 0
    assert completed_flow.header.counts.running == 0
  ...
  defp continuation_distinct_target_fixture! do
    ...
    private_target = %DeploymentTarget{
      ...
      descriptor: %{
        "target_id" => private_asset.target_descriptor.target_id,
        "label" => private_asset.target_descriptor.target_id
      }
    }

    fixture = provision_deploy_fixture({version, packages}, [private_target], [compatibility])
  ...
  defp share_repo_sandbox! do
    unless Process.get(:committed_lifecycle), do: Sandbox.mode(Repo, {:shared, self()})
  end
  ...
  defp install_gated_run_store! do
    assert Process.whereis(Runtime) == nil
    test_pid = self()
    ...
    {:ok, gate} =
      Agent.start_link(fn ->
        %{
          seen: %{},
          gates: MapSet.new(),
          reply_losses: MapSet.new(),
          test_pid: test_pid
        }
      end)

    Application.put_env(:favn_storage_postgres, :step_started_gate, gate)
    ...
    on_exit(fn ->
      if Process.alive?(runtime), do: GenServer.stop(runtime)
      if Process.alive?(gate), do: Agent.stop(gate)
      Application.delete_env(:favn_storage_postgres, :step_started_gate)
    end)
  end

  defp gate_step_started!(run_id, ordinal) do
    configure_step_started!(run_id, ordinal, :gates)
  end

  defp lose_step_started_reply!(run_id, ordinal) do
    configure_step_started!(run_id, ordinal, :reply_losses)
  end

  defp configure_step_started!(run_id, ordinal, field) do
    gate = Application.fetch_env!(:favn_storage_postgres, :step_started_gate)

    Agent.update(
      gate,
      &Map.update!(&1, field, fn values -> MapSet.put(values, {run_id, ordinal}) end)
    )
  end

  defp run_event_count(workspace_id, run_id, event_type) do
    assert %{rows: [[count]]} =
             SQL.query!(
               Repo,
               "SELECT count(*) FROM favn_control.run_events WHERE workspace_id = $1 AND run_id = $2 AND event_type = $3",
               [workspace_id, run_id, event_type]
             )
  ...
  defp await_execution_retry!(pid, remaining \\ 300)

  defp await_execution_retry!(_pid, 0),
    do: flunk("coordinator did not schedule persistence retry")

  defp await_execution_retry!(pid, remaining) do
    state = :sys.get_state(pid)

    case Map.get(state, :execution_persist_pending) do
      %{retry: _} ->
        state

      _ ->
        Process.sleep(10)
        await_execution_retry!(pid, remaining - 1)
    end
  end
  ...
  defmodule TransitionFaultStore do
    for {name, arity} <- FavnOrchestrator.Persistence.RunStore.behaviour_info(:callbacks),
        name != :commit_transition do
      args = Macro.generate_arguments(arity, __MODULE__)
      defdelegate unquote(name)(unquote_splicing(args)), to: FavnStoragePostgres.Runs.Store
    end

    def commit_transition(command) do
      gate = Application.fetch_env!(:favn_storage_postgres, :transition_fault_gate)
      ...
      fault? =
        mode not in [:none] and
          (ordinal == 1 or mode in [:permanent, :lost_always, :lost_then_permanent])

      result =
        cond do
          fault? and (mode == :permanent or (mode == :lost_then_permanent and ordinal > 1)) ->
            {:error,
             FavnOrchestrator.Persistence.Error.new(
               :invalid,
               "injected permanent transition rejection"
             )}

          fault? and mode == :reject_once ->
            {:error,
             FavnOrchestrator.Persistence.Error.new(:timeout, "injected unavailable transition",
               retryable?: true
             )}
      ...
      result =
        if fault? and mode in [:lost_once, :lost_always, :lost_then_permanent] and
           ...
           do:
             {:error,
              FavnOrchestrator.Persistence.Error.new(:timeout, "injected committed reply loss",
                retryable?: true
              )},
           else: result

      if ordinal > 0,
        do: send(owner, {:transition_fault, command.event.event_type, ordinal, command})

      result
    end
  end

  defp install_transition_fault!(event, mode) do
    gate =
      start_supervised!(
        {Agent, fn -> %{event: event, mode: mode, count: 0, command_id: nil, owner: self()} end}
      )

    owner = self()
    Agent.update(gate, &%{&1 | owner: owner})
    Application.put_env(:favn_storage_postgres, :transition_fault_gate, gate)
    on_exit(fn -> Application.delete_env(:favn_storage_postgres, :transition_fault_gate) end)

    {:ok, runtime} =
      Runtime.start_link(%Runtime{
        backend: Backend,
        options: [],
        stores: %{Backend.stores() | runs: TransitionFaultStore}
      })
  ...
  for event <- [:run_started, :step_running, :run_finished],
      mode <- [:reject_once, :lost_once, :lost_then_permanent, :permanent, :lost_always] do
    if mode == :lost_always, do: @tag(:slow)
    @tag timeout: 90_000
    @tag committed_lifecycle: true
    @tag lifecycle_fix: true
    @tag transition_event: event
    @tag transition_mode: mode
    test "bounded transition #{event} #{mode}",
         %{transition_event: event, transition_mode: mode} = fixture do
      {run, _} = create_continuation_pipeline_run!(fixture, 1)
      gate = install_transition_fault!(event, mode)
      start_pipeline_runtime!()
      assert {:ok, pid} = RunServer.start_link(%{run_state: run, version: fixture.version})
      Process.unlink(pid)
      monitor = Process.monitor(pid)

      if event != :run_started or mode != :permanent do
        assert [_] = await_runner_task_ids!(fixture.workspace_id, run.id, 1, 4_000)
        count = if event == :step_running and mode == :permanent, do: 1, else: 3

        for n <- 1..count do
          assert {:ok, task} = claim_asset_task(fixture, "transition-fault-#{n}")
          await_runner_task_waiter!(task)
          assert :ok = start_runner_task(task)

          if event == :step_running and n == 1,
            do: assert_receive({:transition_fault, ^event, 1, _}, 5_000)

          :ok = complete_asset_task(task, task.payload, false)
        end
      end

      assert_receive {:DOWN, ^monitor, :process, ^pid, :normal}, 40_000
      assert {:ok, saved} = get_run(fixture, run.id)
      assert is_nil(saved.metadata["recovery_attention"])

      if mode == :permanent do
        assert saved.status == :error
        assert saved.metadata["failure_cleanup"]["state"] == "pending"
        assert Agent.get(gate, & &1.count) == 1
      else
        assert saved.status == :ok
        assert length(saved.result.node_results) == 3
        assert length(runner_task_ids(fixture.workspace_id, run.id)) == 3
        assert active_execution_lease_count(fixture.workspace_id, run.id) == 0
      end
    end
  ...
  for fault <- [:history, :malformed_terminal, :missing_detail] do
    ...
    test "cleanup retains results and unknown write protection after #{fault}", %{
      ...
      start_supervised!({FavnOrchestrator.ExecutionAdmission.Coordinator, []})
      assert {:ok, first} = RunServer.start_link(%{run_state: run, version: fixture.version})
      original_ids = await_runner_task_ids!(fixture.workspace_id, run.id, 3)
      :ok = :sys.suspend(first)
      kill_run_owner!(fixture, run.id, first)

      tasks =
        for n <- 1..3 do
          assert {:ok, task} = claim_asset_task(fixture, "corrupt-history-#{n}")
          assert :ok = start_runner_task(task)
      ...
      unknown = Enum.find(tasks, &(&1.payload.asset_ref == {MyApp.PrivateAsset, :private}))
      lock = unknown.orchestration_context.materialization_claim.target_operation_lock

      assert %{rows: [["outcome_unknown"]]} =
               SQL.query!(
                 Repo,
                 "SELECT effect_state FROM favn_control.materialization_claims WHERE workspace_id=$1 AND effect_task_id=$2",
                 [fixture.workspace_id, unknown.task_id]
               )

      assert {:ok, authority} =
               RunOwnership.claim(fixture.workspace_context, run.id, "cleanup-audit")

      assert {:ok, saved} = get_run(fixture, run.id)
      owned = RunState.with_storage_fence(saved, authority.owner_id, authority.fencing_token)

      padded =
        Enum.reduce(1..55, owned, fn _, previous ->
          next = RunState.transition(previous, status: :running)
      ...
      if malformed? do
        ...
        {:ok, encoded} = FavnOrchestrator.RunnerTaskContext.encode(%{})

        SQL.query!(
          Repo,
          "UPDATE favn_control.runner_tasks SET orchestration_context=$3 WHERE workspace_id=$1 AND task_id=$2",
          [fixture.workspace_id, malformed.task_id, encoded]
        )
      end
      ...
      assert {:retry, %{kind: :unavailable}} =
               FailureCleanup.apply_result(
                 checkpointed,
                 {:events, nil, run.id, 50},
                 {:error, FavnOrchestrator.Persistence.Error.new(:unavailable, "outage")}
               )

      assert {:ok, restarted} =
               FailureCleanup.new(
                 checkpointed_run
                 |> RunState.with_storage_fence(authority.owner_id, authority.fencing_token),
                 fixture.version
               )
      ...
      assert FavnOrchestrator.Storage.JsonSafe.error(finished.run.error) ==
               FavnOrchestrator.Storage.JsonSafe.error(failed.error)
      ...
      assert {:error, %{details: %{reason_code: "target_write_outcome_unknown"}}} =
               TargetOperationLockStore.acquire_many(%AcquireTargetOperationLocks{
                 workspace_context: fixture.workspace_context,
                 command_id: "blocked-replacement:" <> run.id,
                 target_ids: [lock.target_id],
                 operation_id: "replacement:" <> run.id,
                 operation_type: :materialization,
                 lease_owner: "replacement",
      ...
      assert {:ok, candidates} =
               FavnStoragePostgres.RunOwnership.Store.cleanup_candidates(
                 fixture.workspace_context,
                 64
               )

      refute run.id in candidates
    end
  end

  defp checkpoint_cleanup!(cleanup, remaining) when remaining > 0 do
    alias FavnOrchestrator.RunServer.FailureCleanup
    operation = FailureCleanup.operation(cleanup)

    assert {:cont, next} =
             FailureCleanup.apply_result(cleanup, operation, FailureCleanup.perform(operation))
  ...
  defp drain_cleanup!(cleanup, remaining, missing_id) when remaining > 0 do
    ...
    case FavnOrchestrator.RunServer.FailureCleanup.apply_result(cleanup, operation, result) do
      ...
      other ->
        flunk("cleanup did not converge: #{inspect({elem(operation, 0), other}, limit: 20)}")
    end
  end
end
