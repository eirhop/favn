defmodule FavnRunner.ExecutionAdmission do
  ...
  Work is rejected with a safe retryable error when active capacity is
  exhausted. The orchestrator owns durable queueing and retry policy.
  """
  ...
  def new(opts \\ []) when is_list(opts) or is_map(opts) do
    ...
    %__MODULE__{max_active_workers: max_active_workers}
  end
  ...
  """
  @spec diagnostics(t(), non_neg_integer()) :: map()
  def diagnostics(%__MODULE__{} = admission, active_worker_count)
      when is_integer(active_worker_count) and active_worker_count >= 0 do
    %{
      active_worker_count: active_worker_count,
