defmodule FavnRunner.ExecutionSQLAssetTest do
  ...
  test "shares resolved parameters with transactional SQL checks" do
    ...
    assert_received {:checked_query,
                     "SELECT * FROM (SELECT ? = ? AS passed) AS favn_check LIMIT 2"}
  ...
  defp work_for(version, ref, run_id) do
    original = execution_package_for(version)
    published = original |> Favn.Manifest.Serializer.encode_manifest!() |> Jason.decode!()
    assert {:ok, loaded} = ExecutionPackage.from_published(published)
...
defmodule FavnRunner.ExecutionSQLAssetTest.FakeInspectionAdapter do
  ...
  def disconnect(:conn, _opts) do
    if pid = Application.get_env(:favn_runner, :execution_sql_asset_test_pid) do
      send(pid, :inspection_disconnected)
    end
