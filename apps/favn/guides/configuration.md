...
| `:runner_image` | none | Existing customer-built image. `--runner-image` overrides config; `FAVN_RUNNER_IMAGE` is the fallback. |
...
an environment value and running `mix favn.reload` recreates the existing runner
image when required. A `config/runtime.exs` code change is part of the immutable
...
`favn_duckdb_adbc` plugin loads that path but Favn does not choose, download, or
install the native DuckDB driver. The operator must supply a compatible driver
in every runner image that enables the plugin.

For local Docker Compose development, mount an installed driver at the same
path, read-only:
...
services:
  runner:
    volumes:
      - type: bind
        source: /opt/duckdb/1.5.2/libduckdb.so
        target: /opt/duckdb/1.5.2/libduckdb.so
        read_only: true
```

Add the pinned driver to the customer-owned runner Dockerfile generated by
`mix favn.init --target runner`:

```dockerfile
# In the final runtime stage
COPY --chown=10001:10001 \
  deploy/favn-runner/vendor/libduckdb.so \
  /opt/duckdb/1.5.2/libduckdb.so
```

Build and deploy the resulting image by digest. Keep the configured path, driver
version, target architecture, and native dependencies aligned. A path that
exists on the build host is not available inside the runner unless the final
image contains it or the deployment mounts it.
