defmodule Favn do
  ...
  A typical non-runtime inspection flow is:
      ...
      runner_releases = %{"default" => runner_release_id}
  ...
  `publication.version` is the compact schema-14 manifest index. It binds each
  logical runner pool to an exact image through `runner_releases`.
  SQL assets point to immutable entries in `publication.execution_packages` by
  content hash. Each release ID is the immutable identity generated by local
  tooling or chosen by production CI for one customer runner image.
  ...
  `:runner_releases` is required and maps every effective logical runner pool
  to its exact immutable runner release.
  ...
  ## Example
      ...
      {:ok, manifest} = Favn.generate_manifest(runner_releases: %{"default" => runner_release_id})
  ...
  required `:runner_releases` map prepared for the customer runner images.
  """
