defmodule Favn.Dev.Init.Runner do
  ...
  contract or Favn-owned build pipeline.
  """

  alias Favn.Dev.Paths

  @default_output "deploy/favn-runner"
  ...
  def run(opts) when is_list(opts) do
    with {:ok, root_dir} <- project_root(opts),
         ...
         targets <- targets(output, app, output_relative, project_root_relative),
  ...
  defp targets(output, app, output_relative, project_root_relative) do
    [
      {Path.join(output, "Dockerfile"), dockerfile(output_relative, project_root_relative)},
  ...
  defp dockerfile(output_relative, project_root_relative) do
    """
    ...
    RUN mix deps.get --only prod --check-locked \
        && mix deps.compile \
        && if [ -d #{project_root_relative}/config/runtime ]; then \
             mkdir -p rel/overlays/releases/#{@release_version}/runtime; \
             cp -R #{project_root_relative}/config/runtime/. rel/overlays/releases/#{@release_version}/runtime/; \
           fi \
