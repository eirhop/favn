defmodule Favn.Dev.ConsumerConfigTransport do
  ...
  def bootstrap_eval_snippet do
    ...
    defmodule Favn.Dev.ConsumerConfigBootstrap do
      @supported_keys ~w(connection_modules connections runner_plugins duckdb_in_process_client)
      ...
      defp decode_entries([%{"key" => key, "value" => value} | rest], acc)
           when key in @supported_keys do
        with {:ok, decoded} <- decode_value(value) do
          decode_entries(rest, [{String.to_atom(key), decoded} | acc])
        end
      end

      defp decode_entries([%{"key" => key} | _rest], _acc), do: {:error, {:unsupported_key, key}}
      defp decode_entries(_entries, _acc), do: {:error, :invalid_payload}

      defp decode_value(%{"$type" => "atom", "value" => value}) when is_binary(value),
        do: {:ok, String.to_atom(value)}

      defp decode_value(%{"$type" => "tuple", "items" => items}) when is_list(items) do
        with {:ok, decoded} <- decode_list(items), do: {:ok, List.to_tuple(decoded)}
      end

      defp decode_value(%{"$type" => "list", "items" => items}) when is_list(items),
        do: decode_list(items)

      defp decode_value(%{"$type" => "map", "entries" => entries}) when is_list(entries) do
        entries
        |> Enum.reduce_while({:ok, []}, fn
          %{"key" => key, "value" => value}, {:ok, acc} ->
            with {:ok, decoded_key} <- decode_value(key),
                 {:ok, decoded_value} <- decode_value(value) do
      ...
      defp decode_value(value) when is_binary(value) or is_number(value) or is_boolean(value) or is_nil(value),
        do: {:ok, value}

      defp decode_value(_value), do: {:error, :invalid_payload}
  ...
  defp encode_value(value) when is_atom(value), do: %{"$type" => "atom", "value" => Atom.to_string(value)}
  ...
  defp decode_entries([%{"key" => key, "value" => value} | rest], acc)
       when key in ~w(connection_modules connections runner_plugins duckdb_in_process_client) do
    with {:ok, decoded} <- decode_value(value) do
      decode_entries(rest, [{String.to_atom(key), decoded} | acc])
    end
  end
  ...
  defp sensitive_key?(key) when is_binary(key) do
    ...
    Enum.any?(~w(password passwd pwd token secret database_url connection_string url), &String.contains?(normalized, &1))
  end
