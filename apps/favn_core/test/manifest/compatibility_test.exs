defmodule Favn.Manifest.CompatibilityTest do
  ...
  test "rejects the previous schema version" do
    manifest = %{schema_version: 4, runner_contract_version: 5, assets: []}

    assert {:error, {:unsupported_schema_version, 4, 5}} =
             Compatibility.validate_manifest(manifest)
  end

  test "rejects unsupported schema version" do
    manifest = %{schema_version: 9, runner_contract_version: 5, assets: []}

    assert {:error, {:unsupported_schema_version, 9, 5}} =
             Compatibility.validate_manifest(manifest)
  end

  test "rejects unsupported runner contract version" do
    manifest = %{schema_version: 5, runner_contract_version: 7, assets: []}

    assert {:error, {:unsupported_runner_contract_version, 7, 5}} =
             Compatibility.validate_manifest(manifest)
  end
  ...
  test "rejects namespaced SQL definitions with the previous runner contract" do
    ...
    assert {:error, {:unsupported_runner_contract_version, 4, 5}} =
             Compatibility.validate_manifest(manifest)
  end

  test "rejects missing schema version" do
    manifest = %{runner_contract_version: 5, assets: []}
  ...
  test "rejects missing runner contract version" do
    manifest = %{schema_version: 5, assets: []}
