defmodule Favn.Manifest.SerializerTest do
  ...
  test "encodes canonical json with sorted keys" do
    ...
    assert encoded ==
             ~s|{"a":2,"connection_circuits":{},"environment":{"coverage_scope":null,"default_timezone":"Etc/UTC","default_timezone_source":"utc_fallback"},"execution_pools":{},"runner_contract_version":#{Compatibility.current_runner_contract_version()},"runner_releases":{},"schema_version":#{Compatibility.current_schema_version()},"z":1}|
  end
  ...
  test "uses build manifest payload when build struct is provided" do
    ...
    assert decoded["runner_releases"] == %{}
