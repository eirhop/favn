defmodule Favn.Manifest.VersionTest do
  ...
  test "fails when schema version is unsupported" do
    ...
    assert {:error, {:unsupported_schema_version, 0, 20}} =
             Version.new(manifest)
  end
  ...
  test "schema 12 rejects removed asset lookback fields" do
    manifest =
      current_manifest(%{
        assets: [
          %{
  ...
  test "schema 12 rejects unresolved asset, pipeline, and schedule timezones" do
  ...
  test "schema 12 requires a matching descriptor for persisted SQL targets" do
  ...
  test "schema 12 requires the canonical semantic generation for windowed assets" do
  ...
  test "pins canonical manifest payload when input is build" do
    ...
    assert version.manifest.schema_version == 20
    assert version.manifest.runner_contract_version == 16
  ...
  test "verifies legacy published manifest ids without minting a new identity" do
    ...
    assert {:ok, verified} =
             Version.from_published(original.manifest,
               ...
               runner_releases: original.runner_releases,
  ...
  test "envelope versions are derived from manifest payload" do
    ...
    assert version.runner_releases == manifest.runner_releases
  end
  ...
  test "rejects version override options" do
    ...
    assert {:error, {:unknown_opt, :schema_version}} =
             Version.new(manifest, schema_version: 13)
  end
  ...
  test "published envelopes require and match the runner release identity" do
    ...
    assert {:error, {:invalid_runner_releases, nil}} =
             Version.from_published(manifest,
               manifest_version_id: version.manifest_version_id,
               content_hash: version.content_hash
             )

    alternate = %{"default" => FavnTestSupport.runner_release_id(:alternate)}
    required = manifest.runner_releases

    assert {:error, {:manifest_runner_releases_mismatch, ^alternate, ^required}} =
             Version.from_published(manifest,
               manifest_version_id: version.manifest_version_id,
               content_hash: version.content_hash,
               runner_releases: alternate
             )
  end
  ...
  test "verification rejects an envelope whose runner release identity was changed" do
    ...
    assert alternate == %{"default" => FavnTestSupport.runner_release_id(:alternate)}
    assert required == %{}
  end
  ...
  test "round trips group replacement materialization through published JSON" do
    materialization =
      {:incremental, strategy: :replace_groups, replacement_key: [:tenant_id, :customer_id]}

    manifest = manifest_with_materialization(materialization)
    assert {:ok, encoded} = Serializer.encode_manifest(manifest)
    assert {:ok, decoded} = Serializer.decode_manifest(encoded)

    assert {:ok, version} =
             Version.new(decoded, manifest_version_id: "mv_group_replacement")
  ...
  test "rehydrates decoded manifests into canonical runtime structs" do
    ...
    contract =
      Contract.new!(%{
        ...
        compositions: [
          Composition.new!(MyApp.Contracts.RecordIdentity, 0, [:id])
        ],
        unique_keys: [[:id]],
        row_counts: [[equals: Param.new!(:expected_rows), on_violation: :warn]]
      })
    ...
    assert %Contract{
             ...
             compositions: [
               %Composition{
                 module: MyApp.Contracts.RecordIdentity,
                 start_index: 0,
                 columns: [:id]
               }
             ],
    ...
    invalid_row_count =
      put_in(
        package_decoded,
        ["sql_execution", "contract", "row_counts", Access.at(0), "min"],
        1
      )
    ...
    noncanonical_contract =
      update_in(package_decoded, ["sql_execution", "contract"], fn contract ->
        contract
        |> Map.put("row_count", List.first(contract["row_counts"]))
        |> Map.delete("row_counts")
      end)

    assert {:error,
            {:invalid_manifest_payload,
             %ArgumentError{message: "invalid SQL output contract fields"}}} =
             ExecutionPackage.from_published(noncanonical_contract)
  ...
  test "round trips multiple ordered row-count claims through execution packages" do
    ...
    assert {:ok, package} = ExecutionPackage.new(ref, execution)
    assert package.schema_version == 6
  ...
  defp current_manifest(overrides \\ %{}) do
    ...
    if Map.has_key?(overrides, :runner_releases) do
      manifest
    else
      pools =
        manifest.assets
      ...
      releases =
        Map.new(pools, fn pool ->
          {Atom.to_string(pool), FavnTestSupport.runner_release_id()}
        end)

      Map.put(manifest, :runner_releases, releases)
    end
  end

  defp manifest_environment do
    %{
