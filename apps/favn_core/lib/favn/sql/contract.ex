defmodule Favn.SQL.Contract do
  ...
  alias Favn.SQL.Contract.{Column, Composition, Grain, Param, Relationship, RowCount, UniqueKey}
  ...
  def new!(fields) when is_map(fields) or is_list(fields) do
    ...
    %__MODULE__{
      ...
      relationships: Enum.map(Map.get(fields, :relationships, []), &Relationship.new!/1)
    }
    |> validate!()
  end
  ...
  def validate!(%__MODULE__{} = contract) do
    ...
    unless is_list(contract.relationships) and length(contract.relationships) <= 32,
      do: raise(ArgumentError, "SQL contracts support at most 32 relationships")

    Enum.each(contract.relationships, &Relationship.validate_source!(&1, contract))
    roles = Enum.map(contract.relationships, & &1.name)

    if length(roles) != length(Enum.uniq(roles)),
      do: raise(ArgumentError, "duplicate relationship role")
  ...
  def generated_check_specs(%__MODULE__{} = contract) do
    ...
    row_count_specs(contract) ++
      non_null_specs(contract) ++
      unique_specs(contract) ++
      Enum.flat_map(contract.relationships, &Relationship.check_specs/1)
  end
  ...
  defp validate_fields!(fields) do
    allowed = [:grain, :columns, :compositions, :unique_keys, :row_counts, :relationships]
  ...
  defp validate_generated_check!(%Check{} = check, spec) do
    ...
    matches? =
      check.name == spec.name and
        ...
        unresolved_template(check.template) == expected_template and
        check.uses_query? == (spec.at == :before_materialize) and
        check.uses_target? == (spec.at == :after_materialize)

    unless matches?,
      do: raise(ArgumentError, "SQL contract-generated check #{spec.claim_id} was modified")
  end

  defp unresolved_template(%Template{} = template) do
    %{
      template
      | nodes:
          Enum.map(template.nodes, fn
            %Template.AssetRef{} = ref -> %{ref | resolution: :deferred, relation: nil}
            node -> node
          end)
    }
  end
