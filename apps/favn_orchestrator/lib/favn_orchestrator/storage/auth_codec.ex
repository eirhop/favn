defmodule FavnOrchestrator.Storage.AuthCodec do
  ...
  def encode_credential(%{password_hash: "$argon2" <> _ = password_hash}) do
    {:ok,
     Jason.encode!(%{
  ...
  defp credential_from_dto(%{
         "kind" => "password_hash",
         "algorithm" => "argon2id",
         "password_hash" => "$argon2" <> _ = password_hash
       }) do
  ...
  defp details_from_dto(details) when is_map(details), do: {:ok, details}
