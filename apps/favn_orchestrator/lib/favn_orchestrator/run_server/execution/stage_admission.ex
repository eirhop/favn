defmodule FavnOrchestrator.RunServer.Execution.StageAdmission do
  ...
  def adopt_operation(%{phase: phase, ctx: ctx} = pause, result)
    ...
    case ExecutionAdmission.resolve_admission(ctx.current_run, pause.admission_entry, result) do
      {:ok, lease} -> %{pause | ctx: Map.put(ctx, :lease, lease)}
      {:waiting, waiter} -> %{pause | ctx: Map.put(ctx, :waiter, waiter)}
      _ -> pause
    end
  end

  def adopt_operation(
        %{phase: :materialization_claim, ctx: ctx} = pause,
        %{status: :claimed} = result
      ) do
    {:ok, claim} = MaterializationClaims.resolve_claim(ctx.prepared_claim, result)
    %{pause | ctx: Map.put(ctx, :materialization_claim, claim)}
  end
  ...
  def resume_operation(%{ctx: ctx} = pause, result) do
    if deadline_live?(ctx.work.deadline_at) or
      ...
      expire_operation(pause, result)
    end
  end

  @doc false
  def expire_operation(%{ctx: ctx} = pause, result \\ :ok) do
    ctx =
      case {pause.phase, result} do
        {phase, %{lease: lease}}
        when phase in [:admission, :admission_recheck] and not is_nil(lease) ->
          {:ok, owned} =
            ExecutionAdmission.resolve_admission(ctx.current_run, pause.admission_entry, result)

          Map.put(ctx, :lease, owned)

        {:materialization_claim, result} when is_struct(result) ->
          case MaterializationClaims.resolve_claim(ctx.prepared_claim, result) do
  ...
  defp resume_operation_phase(
    ...
    case ExecutionAdmission.resolve_admission(ctx.current_run, entry, result) do
      {:waiting, waiter} ->
  ...
  defp resume_operation_phase(
         %{phase: :admission_recheck, ctx: ctx, admission_entry: entry},
         result
       ) do
    resolved = ExecutionAdmission.resolve_admission(ctx.current_run, entry, result)
    if match?({:ok, _}, resolved), do: ExecutionAdmission.cancel_wait(ctx.waiter)
    handle_capacity_result(ctx, resolved)
  end

  defp resume_operation_phase(%{phase: :materialization_claim, ctx: ctx}, result),
    do: handle_claim_result(ctx, MaterializationClaims.resolve_claim(ctx.prepared_claim, result))
