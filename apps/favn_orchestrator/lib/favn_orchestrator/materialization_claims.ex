defmodule FavnOrchestrator.MaterializationClaims do
  ...
  @spec acquire(RunState.t(), Version.t(), Index.t(), node_key(), map(), map(), RunnerWork.t()) ::
          ...
          | {:error, term()}
  def acquire(run, version, index, node_key, decisions, context, work) do
    with {:ok, claim, command} <-
           prepare_acquire(run, version, index, node_key, decisions, context, work),
         {:ok, decision} <- claim_materialization(claim, command) do
      resolve_claim(claim, decision)
    end
  end
  ...
  def acquire_sequential(run, work) do
    with {:ok, command} <- prepare_sequential(run, work) do
      if command do
        with {:ok, decision} <- Persistence.stores().materialization.claim(command),
             do: resolve_sequential(decision)
      else
        {:ok, nil}
      end
    end
  ...
  def prepare_sequential(%RunState{} = run, %RunnerWork{} = work) do
    ...
    partition = if window, do: FreshnessKey.window!(window.key), else: "latest"

    command = %ClaimMaterialization{
      workspace_context: SystemContext.workspace(run.workspace_id, :materialization_claim),
      command_id: "claim:" <> key,
      claim_key: key,
  ...
  def abandon_sequential(nil), do: :ok

  def abandon_sequential(%{purpose: :ownership_only} = claim) do
    case finish(claim, :released, []) do
      {:ok, _released} -> :ok
  ...
  def fail(%{purpose: :ownership_only}, _reason), do: :ok
