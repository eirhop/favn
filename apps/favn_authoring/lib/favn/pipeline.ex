defmodule Favn.Pipeline do
  ...
  A manual run without `--window` selects the latest complete period at one
  orchestrator-persisted evaluation instant. Selected assets' availability
  delays are applied before that period is resolved. An explicit request such
  as `mix favn.run MyApp.Pipelines.Monthly --window month:2026-03` remains exact.
  ...
  incomplete period containing the occurrence. The schedule anchor does not
  change the manual default. Schedule cadence remains independent: a daily
  schedule may use monthly anchor windows. Pipelines without a `window` clause
  submit no anchor window and are the normal full-load path.
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      Module.register_attribute(__MODULE__, :favn_pipeline_runner_pool, persist: false)
  ...
  @doc """
  Selects the default logical runner environment for pipeline assets.

  Asset-level `runner_pool` declarations override this value. Pool names are
  opaque user-defined atoms; `:default` is used when neither level declares one.
  """
  defmacro runner_pool(name) do
    quote bind_quoted: [name: name] do
      Favn.Pipeline.ensure_in_pipeline_block!(__MODULE__, "runner_pool")

      Favn.Pipeline.ensure_singleton_clause!(
        __MODULE__,
        :favn_pipeline_runner_pool,
        "runner_pool"
      )

      Favn.Pipeline.validate_runner_pool!(name)
      @favn_pipeline_runner_pool name
    end
  end
  ...
  defmacro __before_compile__(env) do
    ...
    definition =
      %Definition{
        ...
        runner_pool: Module.get_attribute(env.module, :favn_pipeline_runner_pool),
  ...
  @doc false
  def validate_runner_pool!(value) do
    case Favn.RunnerPool.validate_source(value) do
      :ok ->
        :ok

      {:error, _reason} ->
        raise ArgumentError, "pipeline clause `runner_pool` must be a valid non-nil atom"
    end
  end
